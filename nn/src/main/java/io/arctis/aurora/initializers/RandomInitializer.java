  ...
  public ComposedWeights initialize(long seed, int inputSize, int hiddenSize, int outputSize) {
    ...
    for (int i = 0; i < inputSize; i++) {
      for (int j = 0; j < hiddenSize; j++)
        weights_input_to_hidden[i][j] = random.nextDouble();
    }

    for (int i = 0; i < hiddenSize; i++)
      biases_hidden[i] = random.nextDouble();

    for (int i = 0; i < hiddenSize; i++) {
      for (int j = 0; j < outputSize; j++)
        weights_hidden_to_output[i][j] = random.nextDouble();
    }

    for (int i = 0; i < outputSize; i++)
      biases_output[i] = random.nextDouble();
